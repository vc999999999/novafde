...
from app.validator import evaluate_validation
...
def test_restores_dropped_user_facts_and_identity_fields() -> None:
    ...
    ir = restore_authoritative_facts(
        SkillIR.model_validate(empty_ir_payload()),
        brief,
        spec,
    )
...
def test_preserves_model_refinements_while_keeping_mandatory_rules_verbatim() -> None:
    ...
    ir = restore_authoritative_facts(SkillIR.model_validate(payload), brief, spec)

    # Model refinements survive.
    assert ir.workflow.objective == "交付带证据链的竞品研究报告"
    assert ir.agentKnowledge.unknownKnowledge == [
        "事实、推断、假设三层分级标注法",
        "区分事实、推断和假设",
    ]
    ...
    assert ir.contextEngineering.references == ["references/domain-knowledge.md"]
    # Only authoritative user/system restrictions remain hard requirements.
    assert ir.quality.hardRestrictions == spec.hardRestrictions
    assert "每个结论必须附带来源链接" in ir.quality.softGuidance
    assert ir.agentKnowledge.relatedSkills == brief.relatedSkills


def test_model_added_hard_restriction_creates_validation_warning(tmp_path) -> None:
    brief = build_brief()
    spec = build_skill_spec(brief, revision=1)
    payload = empty_ir_payload()
    payload["quality"]["hardRestrictions"] = ["模型自行增加的业务限制"]
    ir = restore_authoritative_facts(SkillIR.model_validate(payload), brief, spec)
    render_skill_package(ir, tmp_path)

    items, issues, _score = evaluate_validation(tmp_path, ir, brief, spec)

    assert any(item.ruleId == "RULE-002" for item in items)
    assert any(issue.criterion == "RULE-002" for issue in issues)


def test_special_case_is_restored_even_when_model_added_other_decisions() -> None:
    brief = build_brief()
    brief.specialCases = "来源不足时输出缺口清单"
    spec = build_skill_spec(brief, revision=1)
    payload = empty_ir_payload()
    payload["workflow"]["decisionPoints"] = ["证据冲突时标记分歧"]

    ir = restore_authoritative_facts(SkillIR.model_validate(payload), brief, spec)

    assert ir.workflow.decisionPoints == [
        "证据冲突时标记分歧",
        "来源不足时输出缺口清单",
    ]
